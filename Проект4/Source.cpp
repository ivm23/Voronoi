...
/*pair<Point, Point> s(Point pIn, Point pEd) {
	Point intersectAnsw1{ NULL, NULL };
	Point intersectAnsw2{ NULL, NULL };
	if (pIn.y < pEd.y) {
		intersectAnsw1 = intersect(pIn, pEd, 0, 1, w);
	}
	else {
		intersectAnsw1 = intersect(pIn, pEd, 0, 1, -w);
	}
		if (pIn.x < pEd.x) {
		intersectAnsw2 = intersect(pIn, pEd, 1, 0, h);
	}
	else {
		intersectAnsw2 = intersect(pIn, pEd, 1, 0, -h);
	}
	return pair<Point, Point>(intersectAnsw1, intersectAnsw2);
	}
*/
...
Point FindNeighbour(Point& point, vector<pair<Point, Point>>& pBoundary) {
	Point neighbour{ NULL, NULL };

	for (auto pB = pBoundary.begin(); pB != pBoundary.end(); ++pB) {
