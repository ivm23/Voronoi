...
bool belong_face(Point input_point, Face* voronoi_face) {
	std::pair<Point, Point> ray_points(input_point, Point(input_point.x + 1, input_point.y));
	...
	for each (auto edge in edges) {
		...
		if (check_intersect_point_line_edge(intersect_point, edge_points) && check_intersect_point_ray_edge(intersect_point, ray_points.first)) ++count_of_intersect_point;
	}
