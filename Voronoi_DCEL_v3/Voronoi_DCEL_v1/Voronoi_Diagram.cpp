...
void Voronoi_Diagram::finish_boundary(Face * face, std::list<Half_Edge*> &boundary) {
	...
	if (first_edge->get_twin()->get_origin()->get_vertex_coordinates() != end_edge->get_origin()->get_vertex_coordinates()) {
		...
		Point point;
		int flag = 0;
		std::pair<Point, Point> pair_points_first_edge(first_edge->get_origin()->get_vertex_coordinates(), first_edge->get_twin()->get_origin()->get_vertex_coordinates());
		while (!(first_edge->get_twin()->get_origin()->get_vertex_coordinates().equals(end_edge->get_origin()->get_vertex_coordinates()))) {
			if (flag != 0 && check_intersect_point_line_edge(end_edge->get_origin()->get_vertex_coordinates(), pair_points_first_edge))
				break;
			...
			if (x - ERR <= 0 && 0 <= x + ERR && (flag == 4 || flag == 0)) {
				flag = 2;
				if (x_end - ERR <= 0 && 0 <= x_end + ERR) {
					point.y = y_end;
				}
				else {
					point.y = MAX_FRAME_Y;
				...
				if (x - ERR <= MAX_FRAME_X && MAX_FRAME_X <= x + ERR && (flag == 1 || flag == 0)) {
					flag = 3;
					if (x_end - ERR <= MAX_FRAME_X && MAX_FRAME_X <= x_end + ERR) {
						point.y = y_end;
					}
					else {
						point.y = 0;
					...
					if (y - ERR <= 0 && 0 <= y + ERR && (flag == 3 || flag == 0)) {
						flag = 4;
						if (y_end - ERR <= 0 && 0 <= y_end + ERR) {
							point.x = x_end;
						}
						else {
							point.x = 0;
						...
						if (y - ERR <= MAX_FRAME_Y && MAX_FRAME_Y <= y + ERR && (flag == 2 || flag == 0)) {
							flag = 1;
							if (y_end - ERR <= MAX_FRAME_Y && MAX_FRAME_Y <= y_end + ERR) {
								point.x = x_end;
							}
							else {
								point.x = MAX_FRAME_X;
			...
			if (!new_half_edge->get_origin()->get_vertex_coordinates().equals(new_half_edge_twin->get_origin()->get_vertex_coordinates())) {
			...
			pair_points_first_edge.first = first_edge->get_origin()->get_vertex_coordinates();
			pair_points_first_edge.second = first_edge->get_twin()->get_origin()->get_vertex_coordinates();
		...
		if (next_face != NULL) {
			...
			for each (auto edge in next_face->get_boundary()) {
				...
				if (check_intersect_point_line_edge(intersect_point, edge_points)) {
					if (!intersect_point.equals(first_intersect_point)) {
						half_edge_for_first_intersect_point = edge;
						temp_point = intersect_point;
					}
					if (flag && intersect_point.equals(list_face.front().second)) {}
					else
					{
			...
			if (count_of_points == 1) list_face.pop_front();
			first_intersect_point = temp_point;
		}
		else break;
	}
		...
		if (next_face != NULL) {
			...
			for each (auto edge in next_face->get_boundary()) {
				...
				if (check_intersect_point_line_edge(intersect_point, edge_points)) {
					...
					if (flag && intersect_point.equals(list_face.back().second)) {}
					else
					{
			...
			if (count_of_points == 1) list_face.pop_back();
			second_intersect_point = temp_point;
		}
		else break;
	}
	return list_face;
...
void Voronoi_Diagram::add_new_voronoi_cell(Point point_to_add, bool f) {
	...
	while (pair_edge_point != next_half_edge.end()) {
		...
		Point first_edge_begin = pair_edge_point->first->get_origin()->get_vertex_coordinates();
		Point first_edge_end = pair_edge_point->first->get_twin()->get_origin()->get_vertex_coordinates();
		Point first_point = pair_edge_point->second;
		pair_edge_point++;
		Point second_point = pair_edge_point->second;
		Point second_edge_begin = pair_edge_point->first->get_origin()->get_vertex_coordinates();
		Point second_edge_end = pair_edge_point->first->get_twin()->get_origin()->get_vertex_coordinates();

		if (position_point_relative_straight_line(first_point, second_point, currentPoint) < 0) {
			std::swap(first_point, second_point);
			std::swap(first_edge_end, second_edge_end);
			std::swap(first_edge_begin, second_edge_begin);
		}
		std::list<Half_Edge*> new_boundary;
		while (!edge->get_origin()->get_vertex_coordinates().equals(first_edge_begin)) {
			edge = edge->get_next();
		}
		...
		while (!edge->get_twin()->get_origin()->get_vertex_coordinates().equals(second_edge_end)) {
			if (!edge->get_origin()->get_vertex_coordinates().equals(edge->get_twin()->get_origin()->get_vertex_coordinates()))
